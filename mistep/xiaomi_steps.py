...
import requests,time,re,json,random,argparse
...
if __name__ ==  "__main__":
    ...
    parser = argparse.ArgumentParser(description='小米运动修改步数')
    parser.add_argument('--username', type=str, help='账号')
    parser.add_argument('--password', type=str, help='密码')
    parser.add_argument('--steps', type=str, help='步数')
    parser.add_argument('--pushkey', type=str, help='方糖推送')
    args = parser.parse_args()
    steps_list = args.steps
    setps_array = steps_list.split('-')
    if len(setps_array) == 2:
        args.steps = str(random.randint(int(setps_array[0]),int(setps_array[1])))
    elif str(args.steps) == '0':
        args.steps = ''

    main(args.username, args.password, args.steps, args.pushkey)
